...
describe('AuthService', () => {
  ...
  it('should return user if password and email valid', async () => {
    const user = await service.loginService(loginUserMock);
  ...
  it('should return user if password invalid and email valid', async () => {
    expect(
      service.loginService({ ...loginUserMock, password: '4324' }),
  ...
  it('should return user if email not exist', async () => {
    ...
    expect(service.loginService(loginUserMock)).rejects.toThrowError();
  });
